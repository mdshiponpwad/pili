                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr role="row" class="odd">
                                        <td class="sorting_1">{{ $user->name_en ?? $user->name_bn }}</td>
                                        <td class="sorting_1">
                                            <p class="badge badge-success">{{ $user->role }}</p>
                                        </td>
                                        <td>{{ $user->address_bn ?? $user->address_en }}</td>
                                        <td>{{ $user->email }}</td>
                                        <td>{{ $user->phn_bn ?? $user->phn_en }}</td>
                                        <td>
                                            @if ($user->verified == 0)
                                                <p class="badge badge-warning">যাচাই করা হয়নি</p>
                                            @else
                                                <p class="badge badge-success">যাচাই</p>
                                            @endif
                                        </td>
...
@section('js')
    ...
    <script>
        function updateUser(user) {
            ...
            if (user.name_bn) {
                $('#name').val(user.name_bn);
                $('#phn').val(user.phn_bn);
                $('#address').val(user.address_bn);
            }else{
                $('#name').val(user.name_en);
                $('#phn').val(user.phn_en);
                $('#address').val(user.address_en);
            }
